...
set_ui_and_ux_preferences() {
    ...
    execute 'sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string "Korhal" &&
             sudo scutil --set ComputerName "Korhal" &&
             sudo scutil --set HostName "Korhal" &&
             sudo scutil --set LocalHostName "Korhal"' \
        'Set computer name'

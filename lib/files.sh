...
remove_content_from_file() {
  ...
  sed -i.bak "s|${content}||" "${file}" 1>> ~/.dotfiles.log
    rm -v "${file}.bak" 1>> ~/.dotfiles.log
...
remove_line_from_file() {
  ...
  if [[ "$2" =~ ^-?[0-9]+$ ]]; then
    ...
    sed -i.bak "${line}d" "${file}" 1>> ~/.dotfiles.log
      rm -v "${file}.bak" 1>> ~/.dotfiles.log
  else
    content="$2"

    sed -i.bak "/${content}/d" "${file}" 1>> ~/.dotfiles.log
      rm -v "${file}.bak" 1>> ~/.dotfiles.log
  fi
...
untar() {
  ...
  if [[ ! -d "${out_dir}" ]]; then
    mkdir "${out_dir}" 1>> ~/.dotfiles.log
  fi

  tar -zxvf "${tar_file}" --strip-components 1 -C "${out_dir}" 1>> ~/.dotfiles.log \
    || return 1
}
...
content_block_remove() {
  ...
  sed -i '' "${start_line},${end_line}d" "${file}" 1>> ~/.dotfiles.log
